#!/usr/bin/env bash
set -euo pipefail

# Local test harness for scripts/netinstall.sh.
#
# Builds a fake release (tarball + checksums.txt + checksums.txt.sig) in a
# local directory and runs the netinstaller against it using the local-mirror
# mode (PIMON_RELEASE_BASE_URL pointing at a filesystem path). Verifies that:
#   1. a clean, correctly-signed release installs successfully;
#   2. a tampered tarball is refused (checksum mismatch);
#   3. a tampered checksums.txt is refused (signature verification fails);
#   4. a missing signature is refused (fail closed).
#
# No network access is required. Requires: bash, python3, openssl, sha256sum,
# and a pimon release built via scripts/package-linux.sh (or any staged dir).

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
NETINSTALL_SH="$ROOT/scripts/netinstall.sh"
SIGN_SH="$ROOT/scripts/sign-release.sh"

VERSION="${PIMON_TEST_VERSION:-0.1.0}"
VERSION_TAG="v$VERSION"

# Detect the triple the same way netinstall.sh does (Linux-only here).
arch="$(uname -m)"
case "$arch" in
    x86_64|amd64) arch="x86_64" ;;
    aarch64|arm64) arch="aarch64" ;;
    *) echo "unsupported arch: $arch" >&2; exit 1 ;;
esac
TRIPLE="${arch}-unknown-linux-gnu"
ARTIFACT="pimon-${VERSION}-${TRIPLE}.tar.gz"
STAGE="pimon-${VERSION}-${TRIPLE}"

WORK="$(mktemp -d)"
RELEASE_ROOT="$WORK/release"
RELEASE_DIR="$RELEASE_ROOT/$VERSION_TAG"
INSTALL_DIR="$WORK/install"
KEYS="$WORK/keys"

cleanup() {
    if [[ -n "${PIMON_TEST_KEEP:-}" ]]; then echo "KEEP: $WORK"; return; fi
    rm -rf "$WORK"
}
trap cleanup EXIT

fail() {
    echo "FAIL: $1" >&2
    exit 1
}

pass() {
    echo "PASS: $1"
}

# --- helpers ---------------------------------------------------------------

# Build a release tarball with a binary whose --version output stands in for
# the real pimon. With PREBUILT_BIN set, stage through scripts/package-linux.sh
# so the payload is the real release layout; otherwise fall back to a
# synthetic one.
make_release_tarball() {
    local out="$1"

    if [[ -n "${PREBUILT_BIN:-}" && -f "$PREBUILT_BIN" ]]; then
        local pkg_dir="${PIMON_TEST_PKG_DIR:-$WORK/pkg}"
        mkdir -p "$pkg_dir"
        VERSION="$VERSION" TARGET="$TRIPLE" PREBUILT_BIN="$PREBUILT_BIN" \
            OUT_DIR="$pkg_dir" bash "$ROOT/scripts/package-linux.sh" >/dev/null
        cp "$pkg_dir/$ARTIFACT" "$out"
        return
    fi

    local stage="$WORK/$STAGE"
    mkdir -p "$stage"

    # Synthetic release: a tiny executable reporting the version.
    cat > "$stage/pimon" <<EOF
#!/usr/bin/env bash
echo "pimon $VERSION (fake)"
EOF
    chmod +x "$stage/pimon"
    cat > "$stage/pimon.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=pimon
Exec=pimon
EOF
    : > "$stage/pimon.svg"
    echo "License" > "$stage/LICENSE"
    echo "OFL" > "$stage/OFL.txt"
    cat > "$stage/install.sh" <<'INNER'
#!/usr/bin/env bash
set -euo pipefail
HERE="$(cd "$(dirname -- "$0")" && pwd)"
PREFIX="${PREFIX:?}"
install -Dm755 "$HERE/pimon" "$PREFIX/bin/pimon"
echo "Installed pimon to $PREFIX/bin/pimon"
INNER
    chmod +x "$stage/install.sh"

    tar -czf "$out" -C "$WORK" "$STAGE"
}

write_checksums() {
    # checksums.txt uses two spaces between hash and name (matches netinstall.sh).
    local hash
    hash="$(sha256sum "$RELEASE_DIR/$ARTIFACT" | awk '{print $1}')"
    printf '%s  %s\n' "$hash" "$ARTIFACT" > "$RELEASE_DIR/checksums.txt"
}

run_install() {
    PIMON_RELEASE_BASE_URL="$RELEASE_ROOT" \
        bash "$NETINSTALL_SH_PATCHED" --version "$VERSION" --prefix "$INSTALL_DIR" \
        >"$WORK/out.log" 2>&1
}

# --- setup: generate a keypair and patch the pinned key --------------------

mkdir -p "$RELEASE_DIR" "$INSTALL_DIR" "$KEYS"

# Generate a fresh keypair. The harness patches a temp copy of netinstall.sh to
# pin this generated public key, so the test is self-contained (it does not
# depend on the committed public key matching a secret we can produce here).
openssl genpkey -algorithm ED25519 -out "$KEYS/secret.pem" 2>/dev/null
openssl pkey -in "$KEYS/secret.pem" -pubout -out "$KEYS/public.pem" 2>/dev/null

NETINSTALL_SH_PATCHED="$WORK/netinstall.sh"
python3 - "$NETINSTALL_SH" "$KEYS/public.pem" "$NETINSTALL_SH_PATCHED" <<'PY'
import sys, re
src, pub, dst = sys.argv[1], sys.argv[2], sys.argv[3]
text = open(src).read()
pubkey = open(pub).read().strip()
patched = re.sub(
    r"-----BEGIN PUBLIC KEY-----\n.*?\n-----END PUBLIC KEY-----",
    pubkey,
    text,
    count=1,
    flags=re.S,
)
open(dst, "w").write(patched)
PY

# --- case 1: clean install succeeds ---------------------------------------

make_release_tarball "$RELEASE_DIR/$ARTIFACT"
write_checksums
bash "$SIGN_SH" "$RELEASE_DIR/checksums.txt" "$KEYS/secret.pem" >/dev/null

rc=0
run_install || rc=$?
if [[ $rc -ne 0 ]]; then
    cat "$WORK/out.log" >&2
    fail "clean install should succeed"
fi
[[ -x "$INSTALL_DIR/bin/pimon" ]] || fail "pimon binary not installed"
grep -q "Signature OK" "$WORK/out.log" || fail "expected 'Signature OK' in output"
grep -q "Checksum OK" "$WORK/out.log" || fail "expected 'Checksum OK' in output"
grep -q "pimon $VERSION" "$WORK/out.log" || fail "expected installed binary to report version"
pass "case 1: clean signed release installs"

# --- case 2: tampered tarball is refused ------------------------------------

rm -rf "$RELEASE_DIR" "$INSTALL_DIR"
mkdir -p "$RELEASE_DIR" "$INSTALL_DIR"
make_release_tarball "$RELEASE_DIR/$ARTIFACT"
write_checksums
bash "$SIGN_SH" "$RELEASE_DIR/checksums.txt" "$KEYS/secret.pem" >/dev/null
# Tamper with the tarball AFTER checksums were computed.
printf 'evil payload\n' >> "$RELEASE_DIR/$ARTIFACT"

rc=0
run_install || rc=$?
if [[ $rc -eq 0 ]]; then
    fail "tampered tarball should be refused"
fi
grep -q "checksum mismatch" "$WORK/out.log" || fail "expected checksum mismatch message"
[[ ! -e "$INSTALL_DIR/bin/pimon" ]] || fail "tampered tarball must not be installed"
pass "case 2: tampered tarball refused (checksum mismatch)"

# --- case 3: tampered checksums.txt is refused -----------------------------

rm -rf "$RELEASE_DIR" "$INSTALL_DIR"
mkdir -p "$RELEASE_DIR" "$INSTALL_DIR"
make_release_tarball "$RELEASE_DIR/$ARTIFACT"
write_checksums
bash "$SIGN_SH" "$RELEASE_DIR/checksums.txt" "$KEYS/secret.pem" >/dev/null
# Tamper with checksums.txt AFTER signing (signature will no longer verify).
printf 'deadbeef  %s\n' "$ARTIFACT" >> "$RELEASE_DIR/checksums.txt"

rc=0
run_install || rc=$?
if [[ $rc -eq 0 ]]; then
    fail "tampered checksums.txt should be refused"
fi
grep -q "signature verification FAILED" "$WORK/out.log" || fail "expected signature failure message"
[[ ! -e "$INSTALL_DIR/bin/pimon" ]] || fail "tarball must not be installed on bad signature"
pass "case 3: tampered checksums.txt refused (bad signature)"

# --- case 4: missing signature is refused (fail closed) --------------------

rm -rf "$RELEASE_DIR" "$INSTALL_DIR"
mkdir -p "$RELEASE_DIR" "$INSTALL_DIR"
make_release_tarball "$RELEASE_DIR/$ARTIFACT"
write_checksums
# Intentionally do NOT sign — no checksums.txt.sig present.

rc=0
run_install || rc=$?
if [[ $rc -eq 0 ]]; then
    fail "missing signature should be refused"
fi
grep -q "without a release signature" "$WORK/out.log" || fail "expected missing-signature message"
[[ ! -e "$INSTALL_DIR/bin/pimon" ]] || fail "tarball must not be installed without a signature"
pass "case 4: missing signature refused (fail closed)"

echo ""
echo "All netinstaller integrity/authenticity tests passed."