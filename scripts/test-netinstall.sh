...
TARBALL="pimon-${VERSION}-${TRIPLE}.tar.gz"
...
make_fake_tarball() {
    local stage="$WORK/stage/pimon-${VERSION}-${TRIPLE}"
    rm -rf "$WORK/stage"
    mkdir -p "$stage"
...
mkdir -p "$PREFIX/bin"
printf '#!/usr/bin/env bash\necho "pimon %s (fake)"\n' "$(basename "$0")" > "$PREFIX/bin/pimon"
chmod +x "$PREFIX/bin/pimon"
EOF
    chmod +x "$stage/install.sh"
    tar -czf "$RELEASE_DIR/$TARBALL" -C "$WORK/stage" "pimon-${VERSION}-${TRIPLE}"
}
...
run_install() {
    ...
    PIMON_RELEASE_BASE_URL="$WORK/release" \
